...
- `go run ./cmd/frit orphans` — claims and checkouts that no longer add up:
  claimed but unstaffed, prepared but unstarted, held past its takeover
  window, or gone.
- `go run ./cmd/frit stale --days N` — worktrees whose tip has not moved.
- `--json` parses either.
...
- `go run ./cmd/frit board` shows who holds what, for the wider picture.
