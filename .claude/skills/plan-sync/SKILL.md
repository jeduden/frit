...
1. **Enumerate.** `mdsmith list query 'status: "🔲"' plan/`, again for
   `"🔳"`. `go run ./cmd/frit plans --detail` lists each plan's status off every
   ref for cross-lane context (bare `plans` prints only a roll-up).
2. **Gather evidence per plan.** The id is the filename prefix.
  ...
  - Any phase or task work past the creation commit → 🔳.
...
4. **Ambiguity rule.** Work landed then reverted, a tail task deferred,
   or evidence only on an unmerged branch → do not flip to ✅; report
   it with its evidence lines.
