...
// Alternate modfile for dev tools only. golangci-lint is built from
// this graph via `go tool -modfile=tools/go.mod golangci-lint`, so
// its dependency tree and go-version floor stay out of the module
// graph that `go install` and library consumers resolve. Tidy with
