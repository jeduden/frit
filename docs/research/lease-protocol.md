...
| S80 | local default branch lags its own fetched remote-tracking ref                        | `Gather` fetches `--prune` first (S87), then compares the two; a local `main` still behind the refreshed remote-tracking ref is the "fetch ran, merge did not" problem it reports               |
...
| S94 | a started lane resumes after work advances its beat token   | pending — same checkout, pushed work and epoch preserved; one agent, no takeover wait (plan 2609082010, #186, RESUME, FENCE)                                                        |
