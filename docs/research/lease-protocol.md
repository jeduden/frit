...
| S80 | local default branch lags its own fetched remote-tracking ref                        | `Gather` fetches `--prune` first (S86), then compares the two; a local `main` still behind the refreshed remote-tracking ref is the "fetch ran, merge did not" problem it reports               |
...
| S86 | read verb reads landed evidence off a checkout unfetched since a PR merged           | `Gather` fetches `--prune` before reading, gated by `--fetch` (default on); `--no-fetch` or an offline fetch falls back to the local view, naming staleness when a remote exists (S80/S84)      |
