...
scavenge never destroys work. Unlanded means content the base still
lacks — a squash-merged chain is not the example: its commits are
never ancestors, but nothing on them is lost, so it parks nothing.
The tip joined the name 2026-08-23 by plan 2608212011: two parks from
one lane at different tips then land under different refs instead of
colliding, so a park can never contend with an earlier one from the
same machine. Retries are idempotent CAS.
...
| S43 | origin URL edited mid-lifecycle         | a URL edit voids nothing: the CAS token on the ref is the coordinate, not the URL; the holder renews unbroken; observation keys on repo, id and tip (OBS)                     |
...
| S79 | scavenge deletes a branch a worktree still stands on                                 | a fresh worktree-list read gates every local `update-ref -d`; a branch still checked out keeps its local copy, only the remote delete and park proceed (SCAV); Mint shared it, removed |
| S80 | local default branch lags its own fetched remote-tracking ref                        | `Gather` fetches `--prune` first (S87), then compares the two; a local `main` still behind the refreshed ref is the "fetch ran, merge did not" problem                                 |
...
| S82 | reaped squash-landed branch carries a follow-up commit                               | the branch tip is parked to the rescue ref before `branch -D`; a park that cannot happen refuses the teardown (PARK)                                                                   |
...
| S84 | local default branch normally lags origin, so it is never authoritative for evidence | landed evidence reads origin's default branch via its remote-tracking ref, never a local `main` that normally trails (SCAV; the invariant of S13, S75)                                 |
