...
| S85 | `origin/HEAD` unset, so `DefaultRef` falls back to a local default branch            | `DefaultRef` must reach `refs/remotes/origin/<default>` before any local `main`; else a squash-landed `✅` read off a stale local `main` shows `🔲`, and `release`/`reap` refuse landed work (SCAV; see S54, S80, S84) |
