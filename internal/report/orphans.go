...
// OrphanRepo is one repository's abandoned work, kept in three kinds
// rather than one count: a claimed lane with no checkout means work
// was taken and dropped, an empty worktree means a lane was prepared
// and never started, and a prunable one means the checkout is already
// gone. They call for different responses.
...
func (r OrphanRepo) Any() bool {
	return len(r.Unstaffed) > 0 || len(r.Empty) > 0 ||
		len(r.Prunable) > 0
}
