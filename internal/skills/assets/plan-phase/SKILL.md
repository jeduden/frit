...
- Plan id, or enough of the title to resolve it. In the plan's own
  lane the id is inferred from the branch, so `{{frit}} phase` needs
  no selector; pass one only to act on another plan.
- Optional phase `N`; default is the first phase not at ✅.

## Method

1. **Load the phase.** `{{frit}} phase <id>` bundles the open phase in
   ...
   plan carries per-phase files. `{{frit}} show <id>` gives the Goal
