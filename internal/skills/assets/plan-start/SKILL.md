...
`{{frit}} start` is the rung-three escalation `plan-drive` climbs to
...
**`{{frit}} start <selector> --go --json`** claims the named plan,
stands its lane up through herdr, and dispatches its next open phase.
A plain `{{frit}} start <selector>` composes the escalation without
...
- The named plan starts even when `{{frit}} pick` would rank another
