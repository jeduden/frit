...
3. **Phase 1 is a proving slice** — a minimal end-to-end slice that
   ...
   Declare Phase 1 and only the later phases already certain; append
   the rest once the slice shows the real shape. In `## Tasks`, write
...
4. **Write the file** to the `plan/proto.md` shape: frontmatter (`id`,
   ...
   `## Phase N: <title>` each (RED spec, GREEN sites, gate), an
   `## Execution` table, and `## Acceptance Criteria`. Each phase
   section must be executable without reading the others.
