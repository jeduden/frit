...
| 3     | The Gather rows of landed evidence run for real          | sonnet | `go test ./cmd/frit -run 'TestFeatures/S(80\|87):'` passes with no SKIP; `go test ./...` and golangci-lint clean                                         |
| 4     | S59 runs for real, and the doctor gap is recorded        | sonnet | `go test ./cmd/frit -run 'TestFeatures/S59:'` passes with no SKIP; `go test ./...` and golangci-lint clean                                               |
