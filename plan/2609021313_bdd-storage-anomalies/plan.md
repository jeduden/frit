...
|     | ↳      | S42 and S44 drop `@pending` and run as real scenarios in `cmd/frit/bdd_storage_test.go`, the last two storage rows: a second git remote added to the holder's clone never splits coordination — a renewal still lands on the remote `.frit.yml` declares, read through `repocfg`, and the second carries no work ref (S42); a lease acquired from a fork checkout whose own origin is not the configured coordination remote lands on that configured remote, and the fork's origin carries no work ref for the plan (S44). Both are the first rows the section builds with two remotes in play; seven steps are new, each with its own unit test. `go test ./cmd/frit -run 'TestFeatures/S(42                                                                                                                                                                                                                                                                                                                                                                                            |
<?/catalog?>

## Acceptance Criteria

- [x] No scenario in `features/storage.feature` carries `@pending`;
      `go test ./cmd/frit -run TestFeatures/S` reports S37..S44,
      S67..S69, S71 and S78 as PASS, none as SKIP
- [x] Every step is bound in `cmd/frit/bdd_storage_test.go` or reused
